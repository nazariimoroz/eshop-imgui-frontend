...
struct gp_google_identify_response_t
{
    int payment_id;
    std::string link;
};

struct gp_new_google_payment_body_t
{
    std::string jwt;
    int product_id;
};

struct gp_check_payment_body_t
{
    std::string jwt;
    int payment_id;
};

struct gp_check_payment_response_t
{
    std::string status;
...
void google_payment_t::nt_create_payment(
    ...
    const std::string& jwt)
{
    ...
    gp_new_google_payment_body_t body{
        .jwt = std::move(jwt),
        .product_id = product->id
    };

    const auto result = cli->Post("/api/new_google_payment/",
                                  rfl::json::write(body),
                                  "application/json");
...
std::optional<product_model_t> google_payment_t::nt_refresh_product_info(
    const std::shared_ptr<httplib::Client>& cli,
    const category_model_t& category,
    const product_model_t& product)
{
...
bool google_payment_t::nt_check_req_result_for_create_payment(const httplib::Result& result, const std::string& other_message)
{
...
void google_payment_t::mt_send_payment_created_callback(std::optional<payment_identificator_t>&& payment_identificator,
...
std::tuple<bool, std::string> google_payment_t::check_payment(const payment_identificator_t& payment_identificator)
{
    auto cli = std::make_shared<httplib::Client>(BACKEND_HOST);
    if (!cli->is_valid())
    {
...
void google_payment_t::nt_check_payment(const payment_identificator_t& payment_identificator,
                                        const std::string& jwt,
                                        const std::shared_ptr<httplib::Client>& cli)
{
    gp_check_payment_body_t body{
        .jwt = std::move(jwt),
    ...
    const auto response = rfl::json::read<gp_check_payment_response_t>(result.value().body).value();
...
bool google_payment_t::nt_check_req_result_for_check_payment(const httplib::Result& result,
...
void google_payment_t::mt_send_payment_done_callback(
    std::optional<std::string> product_value,
    const std::string& message)
{
    if (context_window == std::nullopt)
